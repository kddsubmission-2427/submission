...
DEFINE_int32(str_iters, 2, "the number of iterations of streamcom");
...
int main(int argc, char *argv[])
{
    ...
    Globals globals(fin, FLAGS_filename, FLAGS_p, FLAGS_prepartitioner_type, FLAGS_lambda);
...
void start_partitioning(Globals &globals, TwoPhasePartitioner &partitioner, std::string &binedgelist)
{
    ...
    if (!FLAGS_prepartitioner_type.empty())
    {
        ...
        *    }
        ...
        partitioner.perform_prepartition_and_partition(communities, volumes);
    }
    else
    {
        // partition with hdrf
